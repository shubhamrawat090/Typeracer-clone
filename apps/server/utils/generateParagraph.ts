// A local string that can be sent in case the API request fails
const string =
...
// from our local string it takes up 150 words at random and then returns a paragraph from it
...
// Fetches sensible paragraphs from a free api.
export async function generateParagraph() {
  try {
    ...
    // In case our api request fails we take from the local string
    console.log(e);
